...
function computeCenter(vendors: Vendor[]) {
  if (vendors.length === 0) return { lat: 20.5547, lng: 74.5247, zoom: 13 };
...
export default function FullScreenVendorMap({
  ...
  const [mapMode, setMapMode] = useState<"map" | "globe">("globe");
  const mapModeRef = useRef<"map" | "globe">("globe");
  const vendorDataRef = useRef<any[]>([]);
  const mapInitRef = useRef({ lat: 20.5547, lng: 74.5247, zoom: 13, mapTypeId: "roadmap" });
  ...
  useEffect(() => {
    ...
    filteredVendors.forEach((v) => { m[v.id] = v; });
    vendorMapRef.current = m;
  }, [filteredVendors]);

  const handleMessage = useCallback(
    (event: MessageEvent) => {
      const fromMap = iframeRef.current && event.source === iframeRef.current.contentWindow;
      const fromGlobe = globeRef.current && event.source === globeRef.current.contentWindow;
      if (!fromMap && !fromGlobe) return;
      try {
  ...
  const { lat, lng, zoom } = useMemo(() => computeCenter(validVendors), [validVendors]);

  vendorDataRef.current = vendorData;
  mapInitRef.current = { lat, lng, zoom, mapTypeId: "roadmap" };
  mapModeRef.current = mapMode;

  // Called when Google Maps iframe finishes loading
  const sendMapInit = useCallback(() => {
    ...
    cw.postMessage(JSON.stringify({
      ...
      cc: colorMap,
      P: Colors.primary,
      lat: mapInitRef.current.lat,
      lng: mapInitRef.current.lng,
      zoom: mapInitRef.current.zoom,
      mapTypeId: "roadmap",
      fullSize: true,
    }), "*");
  }, [colorMap]);

  // Called when globe iframe finishes loading
  const sendGlobeInit = useCallback(() => {
    const cw = (globeRef.current as any)?.contentWindow;
    if (!cw) return;
    cw.postMessage(JSON.stringify({
      type: "init",
      vendors: vendorDataRef.current,
  ...
  // Push vendor updates to whichever iframe is active
  useEffect(() => {
    const msg = JSON.stringify({ type: "update", vendors: vendorData, cc: colorMap, P: Colors.primary });
    (iframeRef.current as any)?.contentWindow?.postMessage(msg, "*");
    (globeRef.current as any)?.contentWindow?.postMessage(msg, "*");
  }, [vendorData, colorMap]);

  const topInset = Math.max(insets.top, 30);
  ...
  return (
    <View style={styles.container}>
      {/* Globe iframe — always mounted, hidden when in map mode */}
      <iframe
        ref={globeRef as any}
        src={`${getApiUrl()}/api/globe-frame`}
        style={{
          position: "absolute", top: 0, left: 0, width: "100%", height: "100%",
          border: "none", display: mapMode === "globe" ? "block" : "none",
        } as any}
        onLoad={sendGlobeInit}
      />
      {/* Map iframe — always mounted, hidden when in globe mode */}
      <iframe
        ref={iframeRef as any}
        src={`${getApiUrl()}/api/map-frame`}
        style={{
          position: "absolute", top: 0, left: 0, width: "100%", height: "100%",
          border: "none", display: mapMode === "map" ? "block" : "none",
        } as any}
        onLoad={sendMapInit}
      />
      ...
      <View style={[styles.mapToggle, { top: topInset + 10 }]}>
        <Pressable
          style={[styles.mapToggleBtn, mapMode === "map" && styles.mapToggleBtnActive]}
          onPress={() => setMapMode("map")}
        >
          <Ionicons name="map-outline" size={18} color={mapMode === "map" ? Colors.primary : Colors.textLight} />
          <Text style={[styles.mapToggleBtnText, mapMode === "map" && { color: Colors.primary }]}>Map</Text>
        </Pressable>
        <View style={styles.mapToggleDivider} />
        <Pressable
          style={[styles.mapToggleBtn, mapMode === "globe" && styles.mapToggleBtnActive]}
          onPress={() => setMapMode("globe")}
        >
          <Ionicons name="earth-outline" size={18} color={mapMode === "globe" ? Colors.primary : Colors.textLight} />
          <Text style={[styles.mapToggleBtnText, mapMode === "globe" && { color: Colors.primary }]}>3D</Text>
        </Pressable>
      </View>
