...
async function main() {
  ...
  // Also build the web version (for Median WebView / browser access)
  console.log("Building web version for browser/WebView...");
  await new Promise((resolve, reject) => {
    const webExportTmp = path.join(require("os").tmpdir(), "go-bharat-web-export");
    const webEnv = {
      ...process.env,
    ...
    const exportProc = spawn(
      "npx",
      ["expo", "export", "--platform", "web", "--output-dir", webExportTmp],
      { stdio: ["ignore", "pipe", "pipe"], env: webEnv }
    );
    if (exportProc.stdout) {
      exportProc.stdout.on("data", (d) => {
        const out = d.toString().trim();
        if (out) console.log(`[Web] ${out}`);
      });
    }
    if (exportProc.stderr) {
    ...
    exportProc.on("close", (code) => {
      ...
      // Copy web export files to static-build/
      ...
      copyDir(webExportTmp, "static-build");
      console.log("Web build copied to static-build/");
      resolve();
    });
    exportProc.on("error", reject);
  });

  console.log("All builds complete! Deploy to:", baseUrl);
  process.exit(0);
}
